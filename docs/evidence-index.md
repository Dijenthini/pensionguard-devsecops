...
| EVID-05 | `baseline/EVID-05-vuln1-eval.png` | eval vulnerability in code |
| EVID-06 | `baseline/EVID-06-vuln2-xss.png` | XSS vulnerability in code |
| EVID-07 | `baseline/EVID-07-vuln3-idor.png` | IDOR vulnerability in code |
| EVID-08 | `baseline/EVID-08-vuln4-nosql.png` | NoSQL vulnerability in code |
...
| ID | File | What It Shows |
|----|------|---------------|
| EVID-18 | `secured/EVID-18-vuln1-fix.png` | eval fixed (blocked) |
| EVID-19 | `secured/EVID-19-vuln2-fix.png` | XSS fixed (blocked) |
| EVID-20 | `secured/EVID-20-vuln3-fix.png` | IDOR fixed (own data only) |
| EVID-21 | `secured/EVID-21-vuln4-fix.png` | Benefits access denied |
| EVID-22 | `secured/EVID-22-sast-after.png` | SAST comparison (6→0) |

### Pipeline Evidence (Day 2-6)
...
| EVID-26 | `pipeline/EVID-26-pipeline-final-green.png` | Final verified green run |

### Scan Reports

| File | Purpose |
|------|---------|
| `baseline/baseline_sast.txt` | Baseline SAST scan |
| `baseline/baseline_only_fixed.txt` | Baseline (4 files) |
| `secured/secured_only_fixed.txt` | Secured (4 files) |
| `secured_sast.txt` | Secured full scan |

## Traceability Matrix

| Threat | Risk | Vulnerability | Control | Code Location | Test | Evidence |
|--------|------|---------------|---------|---------------|------|----------|
| T2 | MEDIUM | Benefits Access | RBAC | `app/routes/benefits.js` | EVID-17 | EVID-21 |
| T3 | MEDIUM | Stored XSS | URL validation | `app/routes/profile.js` | EVID-14 | EVID-19 |
| T5 | HIGH | IDOR | Session auth | `app/routes/allocations.js` | EVID-15 | EVID-20 |
| T8 | HIGH | eval Injection | Numeric validation | `app/routes/contributions.js` | EVID-13 | EVID-18 |

## Evidence Summary

| Category | Count |
|----------|-------|
| Baseline screenshots | 12 |
| Exploit screenshots | 4 |
| Fix screenshots | 5 |
| Pipeline screenshots | 5 |
| Scan reports | 4 |
| **Total** | **30** |